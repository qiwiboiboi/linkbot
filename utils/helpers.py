    ...
    if message.text == "❌ Отмена":
        ...
        if user:
            ...
            if is_admin:
                ...
                # Для обычного авторизованного пользователя - обычная клавиатура
...
def format_user_list(users: list) -> str:
    ...
    # Проверяем, не слишком ли длинный список
    if len(users) > 10:
        return f"📊 Найдено пользователей: {len(users)}\n\n⚠️ Список слишком длинный для отображения одним сообщением."
    
    # Для отображения списка пользователей нужно получить пароли
    from database import db
    
    report = f"📊 Список всех пользователей ({len(users)}):\n\n"
